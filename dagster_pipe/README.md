...
| `dagster_pipe/`       | A Python directory that contains code for your Dagster repository                 |
| `dagster_pipe_tests/` | A Python directory that contains tests for `dagster_pipe`                      |
...
pyenv virtualenv $PYTHON_VERSION dagster_pipe
pyenv activate dagster_pipe
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name dagster_pipe python=PYTHON_VERSION
conda activate dagster_pipe
```
...
Tests can be found in `dagster_pipe_tests` and are run with the following command:

```bash
pytest dagster_pipe_tests
```

As you create Dagster ops and graphs, add tests in `dagster_pipe_tests/` to check that your
